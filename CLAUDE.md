...
`/plugin install mstack@mstack`. Invoke the skills by their short name (e.g.
`/mstack-plan`) — Claude Code resolves it since the name is unique; the
fully-qualified `/mstack:mstack-plan` works too but isn't needed. Prefer these
over freelancing:

| Skill | When to use |
|---|---|
| `/mstack-plan` | New feature — interactive consultation, writes `.mstack/plans/<slug>.md` |
| `/mstack-review` | Critique a plan, lock decisions, produces approved review doc |
| `/mstack-code` | Execute an approved review, atomic commit per task |
| `/mstack-qa` | Scenario-driven Playwright QA + structured bug report |
| `/mstack-debug` | Root-cause investigation for a specific failure |
| `/mstack-mockup` | Generate static HTML design variants under `.mstack/mockups/` |
| `/mstack-design-system` | Design system inspection / tweaks |
| `/mstack-ux-audit` | UX review against the design system |
| `/mstack-research` | Background research for planning |
| `/mstack-auto` | End-to-end auto pipeline (plan → review → code) |
...
- New feature → `/mstack-plan`, not direct edits to `apps/` or `packages/`.
- Bug report → `/mstack-debug` to root-cause, then plan/review/code if non-trivial.
- Design change → `/mstack-mockup` first to explore visually before touching `src/`.
