...
| 2 | Add Replit dev hosts to `allowedDevOrigins` (incl. `*.kirk.replit.dev` + dynamic `REPLIT_DEV_DOMAIN`) | `apps/web/next.config.mjs` | Next.js 16 blocks cross-origin dev requests by default; the Replit preview is served through a `*.replit.dev` proxy iframe. Newer Replit workspace cluster domains are two levels deep (`*.kirk.replit.dev`), and `*` is a single-level wildcard, so each cluster prefix is listed explicitly and `process.env.REPLIT_DEV_DOMAIN` is appended dynamically as a future-proofing fallback. |
...
| 13 | Add Tailwind `@source` for workspace packages | `apps/web/src/app/globals.css` | Tailwind v4 auto-scan excludes `node_modules` by design, which means pnpm-symlinked workspace packages (e.g. `@mlabs/ui-web`) are invisible to the class scanner. Classes used only in `packages/*/src/**/*.tsx` are silently purged from the build — no error, just missing styles on every fork that adds a `<Button>` / `<Field>` / `<PasswordInput>` from `@mlabs/ui-web`. Fix: one `@source "../../../../packages/*/src/**/*.{ts,tsx}";` directive in `globals.css`. |
| 14 | Pass `trustedOrigins` defaults to Better Auth (Replit-aware) | `packages/auth/src/server.ts`, `apps/web/src/lib/auth/origins.ts`, `apps/web/src/lib/auth/index.ts` | Better Auth auto-trusts `new URL(baseURL).origin` only. On Replit the server runs at `localhost:5000` while the browser hits `https://<replit-domain>`, so every `/api/auth/*` call 403s with `"Invalid origin"`. Cross-port localhost dev (3000 ↔ 5000) hits the same wall. Fix: `buildTrustedOrigins({ replitDevDomain })` injects `http://localhost:3000`, `http://localhost:5000`, and `https://${REPLIT_DEV_DOMAIN}` (when set) into the Better Auth config. Forks deploying to a custom domain that differs from `BETTER_AUTH_URL` can use **`BETTER_AUTH_TRUSTED_ORIGINS`** (comma-separated) — Better Auth reads it natively, no app wiring needed. See `.env.example`. |
