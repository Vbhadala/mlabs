...
```bash
# 1. Install (Node 22 + pnpm 10 — pinned in .nvmrc / package.json)
pnpm install

# 2. Configure your environment — generates BETTER_AUTH_SECRET, writes
#    .env.local, prompts for DATABASE_URL, and offers to run migrations
pnpm setup
...
**Minimum for a working signup:** `DATABASE_URL` (a Neon connection string) +
`BETTER_AUTH_SECRET` (generated by `pnpm setup`). Everything else in
`.env.example` is feature-gated and optional to boot.

After a `DATABASE_URL` is set, apply the schema (this is easy to forget — without
it, signup 500s with `relation "..." does not exist`):

```bash
pnpm db:migrate
```

Not sure what's configured? Run **`pnpm doctor`** — it lists every load-bearing
variable (and what breaks if it's missing) and checks your database connection +
pending migrations.
